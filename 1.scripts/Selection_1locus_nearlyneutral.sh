...
printf "$i\n$(tail -n 20 Selection_1locus_nearlyneutral_output_$i.txt)" > Selection_1locus_nearlyneutral_output_$i.txt

for j in `seq 1 2`; do awk '{print $'$j'}' Selection_1locus_nearlyneutral_output_$i.txt | sed '/^$/d'; done > tmp$i.txt

cat tmp$i.txt > Selection_1locus_nearlyneutral_output_$i.txt
